(ns dns_root_zone.main
  "Kotodama WASM entrypoint for the dns_root_zone clean-room actor (L4) — Clojure port.
  ...
  facts (`dns_root_zone.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "dns_root_zone")
(def tier "L4")
...
(def entity-specs
  [{:entity "Header"        :plural "headers"        :id-prefix "dnsrootz_hea"
    :fields [:qr :opcode :aa :tc :rd :ra :rcode :qdcount :ancount :nscount :arcount]
    :required [:qr]
    :coerce {:opcode :int :rcode :int :qdcount :int :ancount :int :nscount :int :arcount :int :qr :bool :aa :bool :tc :bool :rd :bool :ra :bool}
    :refs {}}
   {:entity "Question"      :plural "questions"      :id-prefix "dnsrootz_que"
    :fields [:qname :qtype :qclass]
    :required [:qname :qtype]
    :coerce {:qtype :int :qclass :int}
    :refs {}}
   {:entity "ResourceRecord" :plural "resourcerecords" :id-prefix "dnsrootz_res"
    :fields [:name :type :class_ :ttl :rdlength :rdata]
    :required [:name :type]
    :coerce {:type :int :class_ :int :ttl :int :rdlength :int}
    :refs {}}
   {:entity "OptRecord"     :plural "optrecords"     :id-prefix "dnsrootz_opt"
    :fields [:extendedRcode :version :flags :rdlength]
    :required [:extendedRcode :version]
    :coerce {:extendedRcode :int :version :int :flags :int :rdlength :int}
    :refs {}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"dns_root_zone.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "dns_root_zone-compat" :tier tier :entities entities} 200])
...
(defn start! [] :dns_root_zone-compat/ready)
