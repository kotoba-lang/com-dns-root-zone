(ns dns_root_zone.main-test
  "Contract + behavioral test for the dns_root_zone-compat L4 actor (cljc port).
...
(deftest healthz
  (let [[body status] (m/healthz)]
    (is (= 200 status)) (is (= "dns_root_zone-compat" (:actor body))) (is (= (set m/entities) (set (:entities body))))))

#?(:clj (defn -main [& _]
          (let [{:keys [fail error]} (run-tests 'dns_root_zone.main-test)]
